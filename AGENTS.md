# AGENTS.md — Operating contract for MeepleMind
...
- **Code review.** Post review findings as inline PR review comments whenever
  the finding applies to a specific changed line. If GitHub blocks submitting a
  formal review (for example, because the authenticated account owns the PR),
  still post line-level PR comments directly; only use a review-summary comment
  for findings that do not map cleanly to a changed line or when inline
  commenting is technically unavailable. Do not leave review findings only as
  chat text.
...
- **AGENTS.md** (this file) — *how* to work: principles, gates, engineering
  guardrails, recipe.
...
game's rules) > AGENTS.md (for process) > PLAN.md (for scope/ordering).
