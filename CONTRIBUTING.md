...
framework/games seam, rules-first game onboarding), see `CLAUDE.md` first —
...
| `fix`      | a bug fix |
| `refactor` | behavior-preserving restructuring (tests green before *and* after — CLAUDE.md H4) |
...
A commit should be one logical, green checkpoint (CLAUDE.md G9) — don't
...
- Before opening: `uv run pre-commit run -a` and `uv run pytest` must pass —
  ...
  coverage gate) and will block merge otherwise (CLAUDE.md G10).
- A phase's hard gates (RULES sign-off G1, seam G2, tests G3, eval G5) must be
  satisfied before its PR is considered done — see `CLAUDE.md`.
